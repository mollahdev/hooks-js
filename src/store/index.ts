...
export const addHook = <T extends string>(
    type: HookType,
    hookName: T,
    callback: Function,
    priority: number,
    ...
    if (!callback) {
        ...
        if (!context) {
            for (i = handlers.length; i--; ) {
                if (
                    handlers[i] &&
                    (handlers[i] as HookObject).callback === callback
                ) {
                    handlers.splice(i, 1);
                }
            }
            ...
            for (i = handlers.length; i--; ) {
                handler = handlers[i];
                if (
                    (handler as HookObject).callback === callback &&
                    (handler as HookObject).context === context
                ) {
                    handlers.splice(i, 1);
                }
            }
